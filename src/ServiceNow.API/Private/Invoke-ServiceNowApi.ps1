    ...
    while ($true) {
        ...
        try {
            if ($Raw -or $OutFile) {
                ...
                if ($OutFile) { return }
                # -- Return the raw bytes as a single object so the pipeline does not unroll it.
                #    RawContentStream returns bytes on both PS editions; .Content is a string on
                #    Windows PowerShell 5.1 with -UseBasicParsing, which would corrupt binary downloads.
