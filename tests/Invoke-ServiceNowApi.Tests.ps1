    ...
    Context 'Request construction' {
        ...
        It 'appends and URL-encodes query parameters' {
            InModuleScope ServiceNow.API {
                ...
                Invoke-ServiceNowApi -Method 'GET' -Path 'api/now/table/incident' -Query @{ sysparm_query = 'my ref' }

                # Invoke-RestMethod binds -Uri as [System.Uri], whose ToString()/-like form unescapes
                # percent-encoding differently across .NET editions (e.g. Windows PowerShell 5.1 unescapes
                # %3D, PowerShell 7 unescapes %20). Assert on OriginalString - the exact string the engine
                # built - so the encoded value is compared verbatim on every edition. A space encodes to
                # %20 identically on all editions.
                Should -Invoke Invoke-RestMethod -Times 1 -ParameterFilter {
                    $UriString = if ($Uri -is [uri]) { $Uri.OriginalString } else { [string]$Uri }
                    $UriString -like '*sysparm_query=my%20ref'
                }
            }
        }
